...
in
{
  /*
    ...
    In the following expressions, `derivation` will not be evaluated:
        ...
        (lazyDerivation { inherit derivation; meta.foo }).meta

    In these expressions, it will:
    ...
    An the following expressions, referring to implementation details,
    are not valid:
  ...
  */
  lazyDerivation =
    ...
    in
    {
      ...
      # so any mistakes there are programmer error and not something that
      # `lazyDerivation` consumers should be able to correct for after the fact.
      # So, it is better to assume correctness here and check correctness only
